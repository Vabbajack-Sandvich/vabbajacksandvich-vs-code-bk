...
rem cant set scale output?

ffmpeg -i 1.mp4 -i 2.mp4 -i 3.mp4 -i 4.mp4 -c:v libx264 -crf 20 -preset ultrafast -r 60 -filter_complex "[0:v][1:v]hstack=inputs=2[top];[2:v][3:v]hstack=inputs=2[bottom];[top][bottom]vstack=inputs=2[v]" -map "[v]" out-2x2-2160.mp4
