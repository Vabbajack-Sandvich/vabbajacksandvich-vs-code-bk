...
@rem mp4 to mp4 with lame encoder
@rem ffmpeg -y -i "%%f" -map_metadata -1 -c copy "%%~nf-md.mp4"
ffmpeg -y -i "%%f" -map_metadata -1 -c copy "%%~nf-md.mp4"
timeout /t 1 /nobreak
@rem move stripped suffixed file to temp
echo "moving - %%~nf-md.mp4"
move /y "%%~nf-md.mp4" "!zdestpath!"
@rem pause
@rem delete original mp4
echo "deleting - %%f"
del /f /q "%%f"
timeout /t 1 /nobreak
)
